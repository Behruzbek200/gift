...
def is_amount_busy(amount):
    ...
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute("SELECT id FROM payments WHERE status = 'pending' AND unique_amount = %s LIMIT 1", (amount,))
        row = c.fetchone()
        return row is not None
    except Exception as e:
        print(f"⚠️ is_amount_busy xato ({amount}): {e}")
        return False  # Xato bo'lsa — band emas deb hisoblash
    finally:
        if conn:
            put_db(conn)
...
def process_humo(amount, src="HUMO"):
    """
    HUMO xabaridan kelgan summani FAQAT PENDING to'lovlar bilan
    solishtiradi va topilsa — avtomatik tasdiqlaydi.

    ⚠️ XAVFSIZLIK:
    - Faqat 'pending' to'lovlar qidiriladi
    - 'expired' to'lovlar HISOBGA OLINMAYDI
    - Vaqti o'tgan to'lov qabul qilinmaydi
    """
    amount = float(amount)
    now_ts = time.time()
    ...
    try:
        ...
        # ⚠️ AVVAL: muddati o'tganlarni 'expired' qilish
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        c.execute("""UPDATE payments SET status = 'expired'
                     WHERE status = 'pending'
                     AND expires_at IS NOT NULL
                     AND expires_at < %s""", (now_str,))
        expired_count = c.rowcount
        if expired_count:
            print(f"⏰ {expired_count} ta to'lov muddati o'tdi (process_humo ichida)")

        # 1) FAQAT PENDING to'lovlarni qidirish
        ...
        # 2) Topilmasa — ±2 diapazonda qidirish (faqat pending)
        if not row:
            c.execute("""SELECT id, user_id, unique_amount, created_at, expires_at
                         FROM payments
                         WHERE status = 'pending'
        ...
        # 3) Ham topilmasa — EXPIRED to'lov borligini tekshirish
        if not row:
            c.execute("""SELECT id, user_id, unique_amount, created_at, expires_at, status
                         FROM payments
                ...
                try:
                    if expired_row:
                        bot.send_message(admin_id,
                            f"⚠️ <b>Muddati oʻtgan toʻlovga toʻlov keldi!</b>\n\n"
                            f"💰 Kelgan summa: <b>{format_money(amount)}</b>\n"
                            f"📡 Manba: <b>{src}</b>\n\n"
                            f"🧾 Expired: <code>#{expired_row['id']}</code>\n"
                            f"👤 User: <code>{expired_row['user_id']}</code>\n"
                            f"📅 Yaratilgan: {expired_row['created_at']}\n"
                            f"⏰ Muddati: {expired_row['expires_at']}\n\n"
                            f"⚠️ <b>Vaqti oʻtgan — avtomatik tasdiqlanmadi.</b>\n"
                            f"Qoʻlda tasdiqlash: /confirm {expired_row['id']}")
                    else:
                        bot.send_message(admin_id,
                            f"⚠️ <b>Avto-toʻlov: mos pending topilmadi</b>\n\n"
                            f"💰 Kelgan summa: <b>{format_money(amount)}</b>\n"
                            f"📡 Manba: <b>{src}</b>\n\n"
                            f"User hali «➕ Toʻldirish» qilmagan.")
                except Exception:
                    pass
            return False
...
def kb_back():
    ...
    m.add("🔙 Orqaga")
    return m
# ==================== TO'LOV NUSXALASH KLAVIATURASI ====================
def kb_topup_copy(card, amount, pid):
    """
    To'lov ma'lumotlari uchun nusxalash tugmalari.
    ...
    try:
        m.add(types.InlineKeyboardButton(
            ...
            copy_text=types.CopyTextButton(text=str(card))
        ))
    except Exception:
        # Agar copy_text qo'llab-quvvatlanmasa — eski usul
        m.add(types.InlineKeyboardButton(
            text="📋 Karta raqamini nusxalash",
            callback_data=f"cp_card_{pid}"
        ))

    # Summani nusxalash
    try:
        m.add(types.InlineKeyboardButton(
            text=f"💰 {amount} soʻmni nusxalash",
            copy_text=types.CopyTextButton(text=str(amount))
        ))
    except Exception:
        m.add(types.InlineKeyboardButton(
            text=f"💰 {amount} soʻmni nusxalash",
            callback_data=f"cp_amount_{pid}"
        ))

    # To'lov holatini tekshirish
    m.add(types.InlineKeyboardButton(
        "🔄 Toʻlov holatini tekshirish",
        callback_data=f"chk_pay_{pid}"
    ))
...
def proc_topup(m):
    try:
        # Bekor tekshirish
        if m.text in ["🔙 Orqaga", "❌ Bekor", "❌ Bekor — bekor qilish", "/cancel"]:
        ...
        try:
            ...
            print(f"❌ create_payment xato: {e}")
            import traceback
            traceback.print_exc()
            bot.send_message(m.chat.id,
                f"❌ <b>Toʻlov yaratishda xato</b>\n\n"
                f"<code>{str(e)[:200]}</code>\n\n"
                f"Qayta urinib koʻring.",
                reply_markup=kb_user_acc())
            return
        ...
        # To'lov ma'lumotlarini yuborish (nusxalash tugmalari bilan)
        bot.send_message(m.chat.id,
            ...
            f"💡 <i>Quyidagi tugmalar orqali nusxalashingiz mumkin</i>",
            reply_markup=kb_topup_copy(card, amt, pid))

        print(f"✅ To'lov yaratildi: user={uid} amount={amt} pid=#{pid}")

    except Exception as e:
        # Global xato ushlagich
        print(f"❌❌ proc_topup GLOBAL XATO: {e}")
        import traceback
        traceback.print_exc()
        try:
            bot.send_message(m.chat.id,
                ...
                f"<code>{str(e)[:200]}</code>\n\n"
                f"Adminga murojaat qiling.",
                reply_markup=kb_user_acc())
        except Exception:
            pass
# ==================== TARIX SAHIFALASH KLAVIATURASI ====================
def kb_history(page, total_pages):
    """To'lov tarixi uchun sahifalash tugmalari"""
    m = types.InlineKeyboardMarkup(row_width=3)
    nav = []
    if page > 0:
        nav.append(types.InlineKeyboardButton("⬅️", callback_data=f"hist_{page-1}"))
    nav.append(types.InlineKeyboardButton(f"📄 {page+1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        nav.append(types.InlineKeyboardButton("➡️", callback_data=f"hist_{page+1}"))
    if nav:
        m.row(*nav)
    m.add(types.InlineKeyboardButton("🔄 Yangilash", callback_data=f"hist_{page}"))
    m.add(types.InlineKeyboardButton("🔙 Yopish", callback_data="hist_close"))
    return m
...
def msg_history(m):
    uid = m.from_user.id
    build_history_message(uid, 0, m.chat.id, None)


def build_history_message(uid, page, chat_id, message_id=None, per_page=5):
    """Tarix xabarini yasash va yuborish/tahrirlash"""
    try:
        expire_pending()
    except Exception:
        pass
    ...
    try:
        # Statistika
        c.execute("""SELECT 
                        COALESCE(SUM(CASE WHEN status='paid' THEN 1 ELSE 0 END),0) as paid_n,
                        COALESCE(SUM(CASE WHEN status='paid' THEN unique_amount ELSE 0 END),0) as paid_s,
                        COALESCE(SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END),0) as pend_n,
                        COALESCE(SUM(CASE WHEN status='pending' THEN unique_amount ELSE 0 END),0) as pend_s,
                        COALESCE(SUM(CASE WHEN status='expired' THEN 1 ELSE 0 END),0) as exp_n,
                        COALESCE(SUM(CASE WHEN status='expired' THEN unique_amount ELSE 0 END),0) as exp_s
                     FROM payments WHERE user_id = %s""", (uid,))
        stat = c.fetchone()
        ...
        # Ro'yxat
        c.execute("""SELECT id, unique_amount, status, method, created_at, paid_at
                     FROM payments WHERE user_id = %s
    ...
    text = (
        f"📜 <b>Toʻlov tarixi</b>\n\n"
        f"━━━━━━━━━━━━━━━━\n"
        f"✅ <b>Tasdiqlangan:</b> {stat['paid_n']} ta — "
        f"<b>{format_money(stat['paid_s'])}</b>\n"
        f"⏳ <b>Kutilmoqda:</b> {stat['pend_n']} ta — "
        f"<b>{format_money(stat['pend_s'])}</b>\n"
        f"⏰ <b>Muddati oʻtgan:</b> {stat['exp_n']} ta — "
        f"<b>{format_money(stat['exp_s'])}</b>\n"
        f"━━━━━━━━━━━━━━━━\n\n"
    )

    if not rows:
        text += "📭 <i>Toʻlovlar yoʻq</i>"
    else:
        text += f"<b>Sahifa {page+1}/{total_pages}</b>\n\n"
        for r in rows:
            st_map = {"paid": "✅", "expired": "⏰", "pending": "⏳"}
            st = st_map.get(r["status"], "❓")
            when = r.get("paid_at") or r.get("created_at") or "—"
            text += (
                f"{st} <b>#{r['id']}</b> | {format_money(r['unique_amount'])}\n"
                f"    💳 {r['method']} | 📅 {when[:16]}\n\n"
            )

    mk = kb_history(page, total_pages)
    ...
    if message_id:
        ...
        try:
            ...
            print(f"History send xato: {e}")


# ==================== TARIX SAHIFALASH CALLBACK ====================
@bot.callback_query_handler(func=lambda c: c.data.startswith("hist_") and not c.data.startswith("hist_close"))
def cb_hist(call):
    try:
        page = int(call.data.replace("hist_", ""))
    except Exception:
        bot.answer_callback_query(call.id)
        return
    ...
    uid = call.from_user.id
    bot.answer_callback_query(call.id, "🔄")
    build_history_message(uid, page, call.message.chat.id, call.message.message_id)


@bot.callback_query_handler(func=lambda c: c.data == "hist_close")
def cb_hist_close(call):
    try:
        bot.delete_message(call.message.chat.id, call.message.message_id)
    except Exception:
        pass
...
# ==================== TO'LOV NUSXALASH CALLBACK ====================

@bot.callback_query_handler(func=lambda c: c.data.startswith("cp_card_"))
def cb_copy_card(call):
    """Karta raqami nusxalash (fallback)"""
    pid = call.data.replace("cp_card_", "")
    card = get_setting("card_number", "")
    bot.answer_callback_query(call.id, f"📋 Karta: {card}", show_alert=True)


@bot.callback_query_handler(func=lambda c: c.data.startswith("cp_amount_"))
def cb_copy_amount(call):
    """Summa nusxalash (fallback)"""
    pid = call.data.replace("cp_amount_", "")
    try:
        p = get_payment(int(pid))
        if p:
            bot.answer_callback_query(call.id, f"💰 Summa: {p['unique_amount']}", show_alert=True)
        else:
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("chk_pay_"))
def cb_check_payment(call):
    """To'lov holatini tekshirish"""
    try:
        pid = int(call.data.replace("chk_pay_", ""))
    except Exception:
        bot.answer_callback_query(call.id, "❌ Xato", show_alert=True)
        return
    ...
    p = get_payment(pid)
    if not p:
        bot.answer_callback_query(call.id, "❌ Toʻlov topilmadi", show_alert=True)
        return
    ...
    if status == "paid":
        bot.answer_callback_query(call.id,
            f"✅ Tasdiqlandi!\n💰 {format_money(p['unique_amount'])}\n"
            f"🕒 {p.get('paid_at', '—')}", show_alert=True)
    elif status == "pending":
        bot.answer_callback_query(call.id,
            f"⏳ Kutilmoqda\n💰 {format_money(p['unique_amount'])}\n"
            f"⏰ Muddati: {p.get('expires_at', '—')}", show_alert=True)
    elif status == "expired":
        bot.answer_callback_query(call.id,
            "⏰ Muddati oʻtgan!\nYangi toʻlov uchun «➕ Toʻldirish» ni bosing.",
            show_alert=True)
...
def adm_tg_gifts(m):
    ...
    for g in gs[:40]:
        ...
        # Stiker ma'lumotidan emoji olish
        sticker = g.get("sticker", {}) if isinstance(g, dict) else getattr(g, "sticker", {})
        ...
        # Nomni olish
        name = emoji_names.get(emoji, emoji)

        text += f"• {emoji} <b>{name}</b>\n"
        text += f"   <code>{gid}</code> — ⭐ {sc}\n\n"

    text += "📋 ID ni nusxalab «🔗 Gift bogʻlash» orqali ulang."
