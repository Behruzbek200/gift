...
# ==================== GIFT YUBORISH USULI ====================
# "userbot" — shaxsiy akkaunt (Telethon) ← TAVSIYA
# "bot"     — botning o'z Stars balansidan
# "auto"    — avtomatik tanlash
# "manual"  — admin qo'lda
GIFT_METHOD = os.getenv("GIFT_METHOD", "userbot").lower()
...
DEFAULT_SETTINGS = {
    ...
    "free_gift_referrals": 20,        # C) N ta do'st = 1 tekin
    ...
    # Gift
    "free_gift_id": 0,                # tekin sovg'a ID
    "gift_method": GIFT_METHOD,       # userbot/bot/auto/manual

    # Isbotlar kanali
    "proof_channel_id": "",           # Isbotlar kanal ID (masalan -1001234567890)
    "proof_channel_username": "",     # @username
    "proof_enabled": True,            # yoq/o'chir
    "proof_format": "full",           # full/short/minimal
...
# ==================== QISM 1 TUGADI ====================
# Keyingi QISM 2: Database (Supabase PostgreSQL) — jadvallar, funksiyalar
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 2/17 ====================
# Supabase PostgreSQL pool + jadvallar (avtomatik yaratiladi)
# =====================================================================
...
def refresh_settings_cache():
    ...
    _settings_cache = {}
    _settings_cache_time = 0
    get_setting("bot_active")  # trigger
...
def add_admin_cache(uid):
    """Admin cache'ga qo'shish (yangi admin qo'shilganda)"""
    _admin_cache.add(uid)
...
def register_user(uid, un, fn, ref=0):
    ...
    Referal tizimi A va C avtomatik ishga tushadi.
    """
    conn = get_db()
    c = conn.cursor()
        ...
        if c.fetchone():
            ...
            if not c.fetchone():
                ...
                if get_setting("ref_bonus_on", True):
                    ...
                    # O'chirilgan bo'lsa ham referal tarixga yoziladi (0 bilan)
                    c.execute(
                        """INSERT INTO referral_history (from_user, to_user, amount, created_at)
                           VALUES (%s, %s, 0, %s)""",
                        (ref, uid, now)
                ...
                if get_setting("ref_invites_on", True):
                    c.execute("SELECT referrals_count FROM users WHERE user_id = %s", (ref,))
                    cnt = c.fetchone()["referrals_count"]
                    need = int(get_setting("free_gift_referrals", 20) or 20)

                    if need > 0 and cnt >= need:
                        c.execute(
                            """UPDATE users
                               SET free_gifts = free_gifts + 1,
                                   referrals_count = referrals_count - %s
                               WHERE user_id = %s""",
                            (need, ref)
                        )
                        conn.commit()
                        ...
                        try:
                            bot.send_message(ref,
                                ...
                                f"👥 <b>{need}</b> ta doʻst uchun\n"
                                f"🎁 <b>1 ta tekin sovgʻa</b> berildi!\n\n"
...
# ==================== SOVG'A QO'SHISH ====================
def add_gift(name, emoji, price, stars_price, stock, description=""):
    """Yangi sovg'a qo'shish"""
    conn = get_db()
    c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("""INSERT INTO gifts
                 (name, emoji, price, stars_price, stock, description, is_active, created_at)
                 VALUES (%s, %s, %s, %s, %s, %s, 1, %s)
                 RETURNING id""",
              (name, emoji or "🎁", price, stars_price, stock, description, now))
    gid = c.fetchone()["id"]
    conn.commit()
    put_db(conn)
    return gid
...
def update_gift(gid, field, value):
    ...
    Ruxsat etilgan field'lar: price, stock, stars_price, name, emoji, description, tg_gift_id
    """
    allowed = ("price", "stock", "stars_price", "name", "emoji", "description", "tg_gift_id")
...
def build_gifts_kb(page=0, per_page=8):
    ...
    for g in chunk:
        emoji = g["emoji"] or "🎁"
        btn_text = f"{emoji} {g['name']} — {format_money(g['price'])}"
...
def process_humo(amount, src="HUMO"):
    ...
    try:
        ...
        # 2) Topilmasa — ±2 diapazonda qidirish (yaxlitlash xatolari uchun)
        if not row:
            c.execute("""SELECT id, user_id, unique_amount, created_at, expires_at
                         FROM payments
                         WHERE status = 'pending'
        ...
        # 3) Ham topilmasa — adminga xabar berib chiqish
        if not row:
...
def test_humo_parse():
    ...
    Konsolga natijalarni chiqaradi.
    """
    tests = [
        "🎉 To'ldirish\n➕ 5.000,00 UZS\n📍 ZOOMRAD\n💰 35.000,00 UZS",
        "💰 10.003 UZS",
...
def start_telethon():
    ...
    if not string_session:
        ...
        print("Lokalda generate_session.py ishlatib oling,")
        print("chiqqan STRING ni Environment ga qoʻying.")
        print("=" * 50)
        return
...
def send_gift_auto(uid, tg_gift_id, text=None):
    ...
    if m == "bot":
        ...
        # Bot stars yetmasa — userbot'ga o'tish
...
async def _test_tg_gift():
    """Telethon orqali test gift yuborish"""
    global telethon_client
    if not telethon_client:
        return False, "Telethon ulanmagan"
    try:
...
# ==================== QISM 8 TUGADI ====================
# Keyingi QISM 9: Klaviaturalar — USER (7 tugma, isbot=link)
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 9/17 ====================
# USER klaviaturalar (Admin Panel yo'q, Isbot=kanal link)
# =====================================================================

# ==================== USER ASOSIY MENYU ====================
def kb_user_main(uid):
    """
    User asosiy menyusi — 7 tugma.
    Admin Panel tugmasi YO'Q (user uchun).
    Isbotlar — kanal link uchun.
    """
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    m.add("🎁 Sovgʻalar", "💰 Hisobim")
    m.add("👥 Referal", "📋 Buyurtmalarim")
    m.add("💬 Sharhlar", "📞 Admin")
    m.add("📢 Isbotlar", "🎁 Tekin sovgʻa")
    return m
...
# ==================== QISM 9 TUGADI ====================
# Keyingi QISM 10: Klaviaturalar — ADMIN (14 tugma, isbot=boshqaruv)
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 10/17 ====================
# ADMIN klaviaturalar (14 tugma, isbot=boshqaruv paneli)
# =====================================================================

# ==================== ADMIN ASOSIY MENYU ====================
def kb_admin_main():
    """
    Admin asosiy menyusi — 14 tugma.
    Isbotlar — BOSHQARUV paneli (kanal linki emas!)
...
def kb_admin_gifts():
    ...
    m.add("💵 Narx", "📦 Miqdor")
    m.add("⭐ Stars narxi", "🎯 Tekin belgilash")
...
def kb_admin_ref_set():
    ...
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    m.add("📊 Foiz", "💰 Bonus")
    m.add("🎁 Tekin soni", "🛒 Buyurtma soni")
...
def kb_admin_proof():
    """
    ADMIN uchun — isbotlar kanal BOSHQARUVI.
    (User uchun esa kanal linki bo'ladi)
    """
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
...
def kb_admin_reviews():
    """
    ADMIN uchun sharhlar boshqaruvi.
    (User uchun sharh yozish + ko'rish)
    """
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
...
def kb_admin_gift_method():
    ...
    for x in ["userbot", "bot", "auto", "manual"]:
        st = "✅" if x == cur else "⬜"
...
def kb_admin_ref_toggle():
    ...
    items = [
        ...
        ("ref_invites_on", "👥 Do'st soni → tekin"),
...
def kb_admin_gift_actions(gid):
    ...
    m.add(
        ...
        types.InlineKeyboardButton("❌ O'chirish", callback_data=f"gd_{gid}"),
    )
    return m
...
# ==================== QISM 10 TUGADI ====================
# Keyingi QISM 11: USER handlerlar (start, hisobim, to'ldirish, tarix)
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 11/17 ====================
...
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 12/17 ====================
# USER: Referal (4 submenu), Sovg'alar katalog, sotib olish, tekin
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 12A/17 ====================
# USER: Referal tizimi (havola, do'stlar, balans, statistika)
# =====================================================================
...
def msg_ref_link(m):
    ...
    if s["ref_percent_on"]:
        ...
        n += 1
    if s["ref_invites_on"]:
        rules.append(f"{n}️⃣ Har <b>{s['needed_invites']}</b> ta doʻst → <b>1 tekin sovgʻa</b>")
        n += 1
    if s["ref_orders_on"]:
    ...
    if s["ref_invites_on"]:
        left = max(0, s["needed_invites"] - s["referrals_count"])
        text += f"\n🎯 Keyingi tekin: yana <b>{left}</b> ta doʻst"
    if s["ref_orders_on"]:
        text += f"\n🛒 Buyurtma progress: <b>{s['order_progress']}/{s['needed_orders']}</b>"
...
def msg_ref_stats(m):
    ...
    if s["ref_invites_on"]:
        text += f"🎯 Kerakli: <b>{s['needed_invites']}</b> doʻst = 1 tekin\n"
    if s["ref_orders_on"]:
...
# ==================== QISM 12A TUGADI ====================
# Keyingi QISM 12B: Sovg'alar katalog, sotib olish, buyurtmalar, tekin
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 12B/17 ====================
# USER: Sovg'alar katalog, sotib olish, buyurtmalar, tekin sovg'a
# =====================================================================
...
def cb_confirm_buy(call):
    ...
    try:
        ...
        # Balansdan yechish + stock kamaytirish + buyurtma
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def msg_free_gift(m):
    ...
    need = get_setting("free_gift_referrals", 20)
    rights = u["free_gifts"]
    gid = int(get_setting("free_gift_id", 0) or 0)
    g = get_gift(gid) if gid else None

    if rights < 1:
        bot.send_message(m.chat.id,
            f"❌ <b>Huquq yoʻq</b>\n\n"
            f"🎁 Tekin sovgʻa olish uchun <b>{need}</b> ta doʻst taklif qiling.\n"
            f"👥 Hozir: <b>{u['referrals_count']}</b> ta\n\n"
            f"👉 «👥 Referal» → «🔗 Havolam» orqali doʻst chaqiring.",
            reply_markup=kb_user_main(uid))
        return

    if not g or g["is_active"] != 1:
        bot.send_message(m.chat.id,
            "🎁 Sizda tekin sovgʻa huquqi bor, lekin admin hali sovgʻani belgilamagan.\n"
            "«📞 Admin» orqali murojaat qiling.",
            reply_markup=kb_user_main(uid))
        return

    mk = types.InlineKeyboardMarkup()
    mk.add(types.InlineKeyboardButton(
        f"✅ {g['emoji'] or '🎁'} {g['name']} olish",
        callback_data=f"take_free_{gid}"))

    bot.send_message(m.chat.id,
        f"🎁 <b>Tekin sovgʻa</b>\n\n"
        f"✅ Huquq: <b>{rights} ta</b>\n\n"
        f"Sizga: {g['emoji'] or '🎁'} <b>{g['name']}</b>",
        reply_markup=mk)
...
def cb_take_free(call):
    ...
    try:
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        c.execute("UPDATE users SET free_gifts = free_gifts - 1 WHERE user_id = %s", (uid,))
        c.execute("UPDATE gifts SET stock = stock - 1 WHERE id = %s", (gid,))
        c.execute("""INSERT INTO gift_orders
                     (user_id, gift_id, gift_name, price, is_free, status, created_at)
    ...
    auto_sent = False
    if tg_gift_id:
        ok, _ = send_gift_auto(uid, tg_gift_id, f"🎁 {g['name']} (tekin)")
        if ok:
            auto_sent = True
            conn2 = get_db()
            c2 = conn2.cursor()
    ...
    for a in ADMIN_IDS:
        try:
            mk = types.InlineKeyboardMarkup()
            if not auto_sent:
                mk.add(types.InlineKeyboardButton("✅ Yuborildi", callback_data=f"dlv_{oid}"))
            bot.send_message(a,
...
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 13/17 ====================
# USER: Sharhlar (ko'rish+yozish), Admin yozish, Isbotlar (kanal link)
# =====================================================================
...
def user_proof(m):
    ...
    (Admin uchun esa boshqaruv paneli ochiladi)
    """
    ...
    # Inline URL tugma — bosganda DARHOL kanalga o'tadi
    mk = types.InlineKeyboardMarkup()
...
def default_handler(m):
    """Boshqa hech qanday handler ushlamagan xabarlar"""
    uid = m.from_user.id

    # Rasm/stiker/voice va h.k. — javob bermaydi
    if not m.text:
        return

    # Bot holati
    if not get_setting("bot_active", True) and not is_admin(uid):
        return
    if get_setting("maintenance", False) and not is_admin(uid):
        return
...
# ==================== QISM 13 TUGADI ====================
# Keyingi QISM 14: ADMIN handlerlar (statistika, to'lovlar, sovg'alar CRUD)
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 14/17 ====================
# ADMIN: Statistika, to'lovlar, sovg'alar CRUD
# =====================================================================
...
def proc_add_gift(m):
    ...
    if len(ls) < 5:
        bot.send_message(m.chat.id, "❌ 5 qator kerak.", reply_markup=kb_admin_gifts())
        return
    try:
        name = ls[0]
        emoji = ls[1] or "🎁"
    ...
    gid = add_gift(name, emoji, price, sp, stock)
    bot.send_message(m.chat.id,
        f"✅ <b>Qoʻshildi!</b>\n\n🆔 #{gid}\n{emoji} <b>{name}</b>\n"
        f"💵 {format_money(price)} | ⭐ {sp} | 📦 {stock}",
...
def _adm_gift_field(m, field):
    ...
    try:
        gid = int(ps[0])
        v = float(ps[1].replace(",", "")) if field == "price" else int(ps[1])
        if update_gift(gid, field, v):
            bot.send_message(m.chat.id, f"✅ #{gid} {field} → {v}", reply_markup=kb_admin_gifts())
        else:
...
def adm_pending(m):
    ...
    c = conn.cursor()
    try:
        c.execute("""SELECT id, user_id, gift_name, price, is_free, created_at
    ...
    for r in rows:
        ...
        mk = types.InlineKeyboardMarkup()
        mk.add(types.InlineKeyboardButton("✅ Yuborildi", callback_data=f"dlv_{r['id']}"))
        bot.send_message(m.chat.id,
            f"🧾 №<code>{r['id']}</code>\n🎁 {r['gift_name']} | {tip}\n"
...
# =====================================================================
# ==================== SOVGAMARKET BOT — QISM 15A/17 ====================
# ADMIN: Foydalanuvchilar boshqaruvi + Xabar yuborish (broadcast)
# =====================================================================
...
def adm_ref_set(m):
    ...
    text = (
        ...
        f"👥 N ta doʻst: <b>{get_setting('free_gift_referrals', 20)}</b> — "
        f"{'✅' if get_setting('ref_invites_on', True) else '❌'}\n"
        f"🛒 N ta buyurtma: <b>{get_setting('ref_orders_needed', 3)}</b> — "
...
def r_n(m):
    msg = bot.send_message(m.chat.id,
        f"🎁 <b>Nechta doʻst = 1 tekin sovgʻa</b>\n\n"
        f"Hozirgi: <b>{get_setting('free_gift_referrals', 20)}</b>\n\n"
        f"Yangi son:\n\n❌ Bekor",
        reply_markup=kb_back())
    bot.register_next_step_handler(msg, lambda x: _si(x, "free_gift_referrals", 1, 1000, kb_admin_ref_set))


# ---------- 🛒 BUYURTMA SONI ----------
@bot.message_handler(func=lambda m: m.text == "🛒 Buyurtma soni" and is_admin(m.from_user.id))
def r_o(m):
    msg = bot.send_message(m.chat.id,
        f"🛒 <b>Nechta buyurtma = 1 tekin sovgʻa</b>\n\n"
        f"Hozirgi: <b>{get_setting('ref_orders_needed', 3)}</b>\n\n"
        f"Yangi son:\n\n❌ Bekor",
        reply_markup=kb_back())
    bot.register_next_step_handler(msg, lambda x: _si(x, "ref_orders_needed", 1, 1000, kb_admin_ref_set))


# ==================== YORDAMCHI: INT SOZLAMA ====================
...
# ==================== QISM 15B TUGADI ====================
# Keyingi QISM 16: ADMIN — Obuna, Isbotlar, Stars, Adminlar, Sharh o'chirish, Boshqa
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 16A/17 ====================
...
def adm_add_ch(m):
    msg = bot.send_message(m.chat.id,
        ...
        "Privat kanal boʻlsa — invite link ham yuboring (2-qatorda).\n\n"
...
def adm_stars_menu(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"Method: <b>{get_setting('gift_method', GIFT_METHOD)}</b>\n"
        f"━━━━━━━━━━━━━━━━\n\n"
        f"• <b>userbot</b> — shaxsiy akkaunt (Telethon) ← tavsiya\n"
        f"• <b>bot</b> — botning Stars balansidan\n"
...
def adm_stars_bal(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"Toʻldirish: @BotFather → Payments → Stars",
        reply_markup=kb_admin_stars())
...
# =====================================================================# ==================== SOVGAMARKET BOT — QISM 17/17 ====================
# Flask Webhook (Render) + Startup + Main
# =====================================================================
...
# ==================== WEBHOOK O'RNATISH ====================
def setup_webhook():
    """Render uchun webhook o'rnatish"""
    if WEBHOOK_URL:
        ...
        print("⚠️ WEBHOOK_URL yo'q — polling rejimida ishlaydi")
...
def send_startup_msg():
    ...
    try:
        ...
        proof_ok = "✅" if get_setting("proof_channel_id", "") else "❌"
        ref_status = "✅" if get_setting("ref_percent_on", True) else "❌"
        telethon_status = "✅ Ulandi" if telethon_client else "❌ Oʻchiq"
        mode = "🌐 Webhook (Render)" if WEBHOOK_URL else "📡 Polling"

        text = (
            f"🚀 <b>SovgaMarket Bot ishga tushdi!</b>\n\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"👥 Userlar: <b>{users_n}</b>\n"
            f"🎁 Sovgʻalar: <b>{gifts_n}</b>\n"
            f"📢 Kanallar: <b>{ch_n}</b>\n"
            f"⏳ Kutilayotgan buyurtma: <b>{pending_n}</b>\n"
            f"⭐ Bot Stars: <b>{stars}</b>\n"
            f"🔄 Gift method: <b>{get_setting('gift_method', GIFT_METHOD)}</b>\n"
            f"🔌 Telethon: <b>{telethon_status}</b>\n"
            f"📢 Isbotlar: <b>{proof_ok}</b>\n"
            f"🎁 Referal: <b>{ref_status}</b>\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"🌐 Rejim: <b>{mode}</b>\n"
        ...
        print("✅ Startup xabari yuborildi")
    except Exception as e:
...
def send_shutdown_msg():
    ...
    try:
        text = (
            f"🛑 <b>SovgaMarket Bot toʻxtatildi</b>\n\n"
            f"📅 {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"
        )
        for admin_id in ADMIN_IDS:
            try:
...
def set_bot_commands():
    """Bot menyusidagi buyruqlarni sozlash"""
    try:
        bot.set_my_commands([
        ...
        print("✅ Bot buyruqlari o'rnatildi")
    except Exception as e:
        print(f"❌ Commands xato: {e}")


# ==================== STARTUP ====================
def startup():
    """Botni to'liq ishga tushirish"""
    print("=" * 50)
    print("🎁 SovgaMarket Bot — Render + Supabase")
    print("=" * 50)

    # 1. Database (Supabase PostgreSQL)
    print("📦 DB ishga tushirilmoqda...")
    init_pool()
    init_db()

    # 2. Bot buyruqlari
    print("⚙️ Bot buyruqlari...")
    set_bot_commands()

    # 3. Expire checker (30 sek thread)
    print("⏰ Expire checker...")
    start_expire_checker()

    # 4. Telethon (HUMO 24/7 kuzatuv)
    print("🔌 Telethon...")
    _en_tel = os.getenv("ENABLE_TELETHON", "1" if API_ID else "0")
    if _en_tel == "1" and API_ID and API_HASH:
        start_telethon()
    else:
        print("ℹ️ Telethon o'chiq")

    # 5. Webhook (agar Render bo'lsa)
    if WEBHOOK_URL:
        setup_webhook()

    # 6. Startup xabari (3 sek keyin)
    Thread(target=lambda: (time.sleep(3), send_startup_msg())).start()

    print("=" * 50)
    print("✅ Bot tayyor!")
    print(f"👑 Adminlar: {ADMIN_IDS}")
    print(f"💳 Karta: {CARD_NUMBER}")
    print(f"⭐ Method: {get_setting('gift_method', GIFT_METHOD)}")
    print(f"🌐 Rejim: {'Webhook' if WEBHOOK_URL else 'Polling'}")
    print("=" * 50)


# ==================== POLLING REJIMI (LOKAL) ====================
def run_polling():
    """Lokal kompyuterda polling rejimida ishlatish"""
...
if __name__ == "__main__":
    # ========== VALIDATSIYA ==========
    if not BOT_TOKEN:
        print("❌ BOT_TOKEN yoʻq! Render Environment ga qoʻshing.")
        exit(1)
    if not DATABASE_URL:
        print("❌ DATABASE_URL yoʻq! (Supabase PostgreSQL)")
        exit(1)
    ...
    if WEBHOOK_URL:
        ...
        print(f"🌐 Flask server: 0.0.0.0:{PORT}")
        app.run(
            host="0.0.0.0",
            port=PORT,
            threaded=True,
...
# ==================== GUNICORN UCHUN (RENDER) ====================
else:
    # Gunicorn `app:app` bilan ishga tushganda bu blok ishlaydi
    if BOT_TOKEN and DATABASE_URL:
        try:
            startup()
        except Exception as e:
            print(f"❌ Startup error (gunicorn): {e}")
            import traceback
            traceback.print_exc()


# ==================== TUGADI ====================
# 🎉 SOVGAMARKET BOT TO'LIQ TAYYOR!
# 
# Xususiyatlar:
#   ✅ Supabase PostgreSQL
#   ✅ Render webhook
#   ✅ Telethon HUMO 24/7
#   ✅ Gift avto (userbot/bot)
#   ✅ Referal 4 tizim
#   ✅ Isbotlar kanali (user=link, admin=boshqaruv)
#   ✅ Sharh o'chirish (admin)
#   ✅ Kirim/Chiqim (user)
#   ✅ Admin log
#   ✅ Excel export
#   ✅ Majburiy obuna
#   ✅ Promo kodlar (kelajakda)
# =====================================================================
